...
export const adduser = async (userid: string, name : string) => {
    ...
    const newDoc = userCollectionRef.doc(userid, {"Pokemon": data.name, "URL" : data.other.showdown.front_default, "Pomodoros" : 0, "Name" : name});
    return await newDoc.set({"Pokemon": data.name, "URL" : data.other.showdown.front_default, "Pomodoros" : 0, "Name" : name});
};
